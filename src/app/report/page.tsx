...
export default function ReportList() {
    ...
    const fetchAnswers = async (metadataOptions) => {
    ...
    useEffect(() => {
        ...
        const fetchFilteredData = async () => {
            try {
                const filteredMetadata = await fetchAnswers(metadataOptions);
    ...
    return (
        <div className="max-w-4xl mx-auto mt-4">
            <p className="font-bold flex items-center mb-4">Please select a report to view</p>
                        ...
                        {metadataData.map((item) => (
                            <tr key={item.metadata_id} className="border-b border-gray-200">
                                <td className="w-1/3 border border-gray-300 px-4 py-2 hover:underline">
                                    ...
                                    <Link href={`/report/${encodeURIComponent(item.metadata_id.trim())}`}>
                                        {item.metadata_name}
                                    </Link>
                                </td>
